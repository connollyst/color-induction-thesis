...
\begin{equation}
\begin{cases} 
\dot{x}_{ics\theta} = - \alpha_{x}X_{is\theta} \\
                    & - g_y(y_{ics\theta}) \\
                    & - \sum\nolimits_{\Delta_{s}, \Delta_{\theta} \neq 0} \Psi(\Delta_{s}, \Delta_{\theta}) g_y(y_{is + \Delta_{s\theta} + \Delta_{\theta}}) + J_0(g_x(X_{is\theta})) \\
                    & + \sum\nolimits_{j \neq i,s'\theta'}J_{[is\theta, js'\theta']}g_x()js'\theta') \\
                    & + I_{is\theta} \\
                    & + I_0 \\
\dot{y}_{ics\theta} = - \alpha_{y}Y_{is\theta} \\
                    & + g_x(X_{is\theta}) \\
                    & + \sum\nolimits_{j \neq i,s'\theta'}W_{[is\theta, js'\theta']}g_x()js'\theta') \\
                    & + I_{c}
\end{cases}
\end{equation}
