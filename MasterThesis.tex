...
\begin{equation}
\begin{cases} 
\dot{x}_{i \gamma s\theta} = - \alpha_{x}X_{i \gamma s \theta} \\
                    & - g_y(y_{i \gamma s\theta}) \\
                    & - \sum\nolimits_{\gamma' \neq \text{opp($\gamma$)}, \Delta_{s}, \Delta_{\theta} \neq 0}
                        \Psi(\gamma', \Delta_{s}, \Delta_{\theta})
                        g_y(y_{i \gamma s + \Delta_{s\theta} + \Delta_{\theta}})
                        + J_0(g_x(X_{is\theta})) \\
                    & + \sum\nolimits_{j \neq i,\gamma',s'\theta'}
                        J_{[i \gamma s \theta, j \gamma' s' \theta']}g_x(X_{j \gamma 's'\theta'})) \\
                    & + I_{i \gamma s \theta} \\
                    & + I_0 \\
\dot{y}_{i \gamma s \theta} = - \alpha_{y}Y_{i \gamma s \theta} \\
                    & + g_x(X_{i \gamma s \theta}) \\
                    & + \sum\nolimits_{j \neq i, \gamma' s' \theta'}
                        W_{[i \gamma s \theta, j \gamma' s' \theta']}g_x(X_{j \gamma' s' \theta'}) \\
                    & + I_{c}
\end{cases}
\end{equation}
