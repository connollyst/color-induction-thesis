...
\subsubsection{Single-Opponent Neurons} Processing starts by convoluting each of the L, M, and S cone spectral response functions with two different, concentric, centered, circular gaussian filters. These convolutions simulate integration of information in a neuron's receptive field. One convolution is used to build the center of a cell's receptive field, the other convolution, the surround. In a classical center-surround configuration, the center is smaller and weighed significantly more heavily than the surround, as in Figure \ref{fig:rf-so} (c).
...
\begin{flalign}
    R(c, s) &= max(0, L(c) - M(s)) \\
    G(c, s) &= max(0, M(c) - L(s)) \\
    B(c, s) &= max(0, S(c) - \frac{L(s) + M(s)}{2}) \\
    Y(c, s) &= max(0, \frac{L(c) + M(c)}{2} - S(s))
\end{flalign}

We also introduce two new opponent channels, lightness (L) and darkness (D):

\begin{flalign}
    L(c, s) &= max(0, \frac{L(c) + M(c) + S(c)}{3} - 0.5) \\
    D(c, s) &= | min(0, \frac{L(c) + M(c) + S(c)}{3} - 0.5) |
\end{flalign}
...
\subsubsection{Double-Opponent Neurons} The process of constructing double-opponent color channels is exactly the same as in \textit{Single-Opponent Neurons} above, the only difference being the definition of the gaussian filters used to define the cells' receptive fields. Instead of using two concentric, centered, circular gaussian filters, for double-opponent cells we use asymmetrical, off-center, elongated gaussian filters, like those depicted in Figure \ref{fig:rf-do}. As with the single-opponents cell model, the 'center' and 'surround' (more appropriately referred to as 'excitatory' and 'inhibitory' in the context of double-opponent cells) receptive fields can be adjusted to different sizes, shapes, and weights each. For our tests we worked with symmetric balanced receptive fields, as in Figure \ref{fig:rf-do} (a).

\textbf{// TODO} \textit{reference diagram of double opponent center \& surround}
