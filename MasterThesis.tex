...
To construct opponent color channels, we combine center $(c)$ and surround $(s)$ convolutions of the L, M, and S channels into three pairs: two color \cite{spitzer:2005} and one achromatic.
...
\begin{flalign}
    L(c) &= max(0, \frac{L(c) + M(c) + S(c)}{3} - 0.5) \\
    D(c) &= | min(0, \frac{L(c) + M(c) + S(c)}{3} - 0.5) |
\end{flalign}
