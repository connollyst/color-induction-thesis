...
\begin{itemize}
    \item R(c, s, scale) = on(r(c, scale) - (g(s, scale) + b(s, scale))/2)
    \item G(c, s, scale) = on(g(c, scale) - (r(s, scale) + b(s, scale))/2)
    \item B(c, s, scale) = on(b(c, scale) - (r(s, scale) + g(s, scale))/2)
    \item Y(c, s, scale) = on((r(c, scale) + g(c, scale))/2 - abs(r(s, scale) - g(s, scale))/2 - b(s, scale))
    \item L(c, s, scale) = on(i(c, scale))
    \item D(c, s, scale) = off(i(c, scale))
\end{itemize}
Where \textit{c} indicates the 'center' and \textit{s} indicates the surround, used to define the relationship between center and surround receptive fields. For example, \textit{c} can be given a smaller receptive field than \textit{s}, or \textit{s} can be given a smaller weight than \textit{c}. The \textit{scale} is used to scale both center and surround. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{receptive-fields}
    \caption{Diagrams of receptive fields.}
...
Previous work by Penacchio \textit{et al.} \cite{otazu:plosone} utilized a discrete wavelet transform (DWT) to decompose a greyscale image into it's oriented edges at scale. In the context of our research, this could be thought of as representing achromatic double opponent cells; the response is greatest at luminosity boundaries, and nonexistent on surfaces or in response to chromatic changes.
