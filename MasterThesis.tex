...
\begin{abstract}
We present a computational model of the primary visual cortex (V1) inspired by current neurobiological understanding. This understanding treats color and shape information as intrinsically connected and, as a consequence, predicts perceptual phenomena such as color induction and assimilation to arise very early in visual processing. Our model confirms such predictions on a number of psychophysical experiments, offering credence to the biological theories.
\end{abstract}
...
\begin{enumerate}
    ...
    \item Current view $\rightarrow$ integration of color \& shape
        ...
        \end{itemize}
    \item Z. Li's edge detection work
    \item Xavi's PLoS One 2013 paper
        \begin{itemize}
            \item Avoid detail, save that for \textit{Method}..?
        \end{itemize}
    \item Xavi's JoV 2010 paper; differences
        \begin{itemize}
            \item Weighing Function/Extended CSF vs. Implicit CSF
            \item Mathematical vs Dynamical
            \item .. err, worth mentioning?
        \end{itemize}
    \item \textbf{Any other models?}
\end{enumerate}
